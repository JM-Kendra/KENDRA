...
- **Milestone 11 does not exist.** No branch, commit, ADR, or document defines it.
...
- **The gold dataset (`kendra-bir-public-gold-v2`) is unapproved.** Its
  ...
  false` until the required independent review happens.
...
1. **Append-only audit records** (`question_audit`, Postgres, hash-chained,
   trigger-enforced against `UPDATE`/`DELETE`) for every call through
