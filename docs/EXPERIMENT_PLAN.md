...
**EXP-08 — claimed, draft, not frozen (2026-09-01).** Scope: whether a larger local model answers the specific gold cases where `qwen2.5:7b-instruct` abstained despite the required facts already sitting in its retrieved context (`evaluation/M12_FINDINGS.md` part (d)). This feeds EXP-04's model-selection question above — a narrow signal about whether model size explains a specific class of abstention — rather than replacing or substituting for EXP-04's own schema/latency/memory gates, which remain unaffected and still required. Full draft: [`evaluation/EXP-08_PREREG_DRAFT.md`](../evaluation/EXP-08_PREREG_DRAFT.md). **Not added as a numbered section above and not a registration**, because the claim collides with `ARCHITECTURE.md`'s own `EXP-08` ("Is publication atomic across PostgreSQL and Qdrant?") — a different, unrelated question. The draft records this collision and proposes `EXP-11` as a collision-free alternative; neither this note nor the draft resolves it.
