...
**`docs/EXPERIMENT_REGISTRY.md` is the single source of truth for experiment-ID allocation and status.** Every ID mentioned in this plan, in `ARCHITECTURE.md`, and in `docs/experiment-decisions/` is listed there with its current status and any collision notes; check it before claiming a new ID rather than scanning these documents by hand.

**EXP-11 — claimed, draft, not frozen (2026-09-01).** Scope: whether a larger local model answers the specific gold cases where `qwen2.5:7b-instruct` abstained despite the required facts already sitting in its retrieved context (`evaluation/M12_FINDINGS.md` part (d)). This feeds EXP-04's model-selection question above — a narrow signal about whether model size explains a specific class of abstention — rather than replacing or substituting for EXP-04's own schema/latency/memory gates, which remain unaffected and still required. Full draft: [`evaluation/EXP-11_PREREG_DRAFT.md`](../evaluation/EXP-11_PREREG_DRAFT.md). **Not added as a numbered section above and not a registration.** This claim was checked against `docs/EXPERIMENT_REGISTRY.md` before being made and does not collide with anything currently recorded there; its predecessor claims under `EXP-04` and `EXP-08` did collide and are recorded as resolved-by-rename in the registry.
